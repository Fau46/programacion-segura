...
def log_admin():
    return log_user(username="Admin",password="Admin")

def log_user(username, password="Admin"):
...
def can_vote(dni, allowed):
    query = "UPDATE elector SET can_vote ="+allowed+" WHERE dni="+str(dni)
    ...
    if len(result) == 0:
        ...
        return {}, 200 #may return all user if sqlinjection

def new_elector(dni, firstname, lastname, birthdate):
    """
    Creates a new elector in the database

    Params:
        - dni: the elector's dni
        - firstname: the elector's name
        - lastname: the elector's name
        - birthdate: the elector's birthdate
    """
    elector = add_elector(dni, firstname, lastname, birthdate)
    if elector is None:
...
def new_user(dni, username, password):
    ...
    if user is None:
        ...
        return (user,result), 201#may return all user if sqlinjection
